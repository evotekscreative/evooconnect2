...
export default function Groups() {
  ...
  useEffect(() => {
    const fetchGroups = async () => {
      try {
        ...
        const response = await axios.get(
          "http://localhost:3000/api/my-groups",
          {
            headers: {
    ...
    const fetchInvitations = async () => {
      try {
        const response = await axios.get(
          "http://localhost:3000/api/my-invitations",
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
  ...
  const handleLeaveGroup = async (groupId) => {
    if (
      window.confirm(
        "Are you sure you want to leave this group? You'll need to be invited to join again."
      )
    ) {
      try {
        await axios.post(
    ...
    try {
      ...
      if (response.data && response.data.group) {
        ...
        setGroups((prev) => [
          ...
          {
            id:
              groupData.id || `temp-${Math.random().toString(36).substr(2, 9)}`,
  ...
  const handleRejectInvitation = async (invitationId) => {
    try {
      await axios.post(
        `http://localhost:3000/api/invitations/${invitationId}/reject`,
        {},
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const adminGroups = Array.isArray(groups)
    ? groups.filter((group) => group.is_admin)
    : [];
  ...
  const showcaseAdminGroups = showAllAdminGroups
    ? adminGroups
    : adminGroups.slice(0, 3);
  const showcaseJoinedGroups = showAllJoinedGroups
    ? joinedGroups
  ...
  if (loading) {
    return (
      <div
        className={`${
          group.isAdmin ? "bg-blue-100" : "bg-gray-200"
        } w-10 h-10 sm:w-12 sm:h-12 rounded-full overflow-hidden flex items-center justify-center`}
      >
        <span
          className={`${
            group.isAdmin ? "text-blue-600" : "text-gray-600"
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-4 gap-4 sm:gap-6">
                  ...
                  <div className="mb-8">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-4">
                      ...
                      {adminGroups.length > 3 && (
                        <button
                          onClick={() =>
                            setShowAllAdminGroups(!showAllAdminGroups)
                          }
                        ...
                        .map((invitation) => (
                          ...
                          >
                            <div className="flex items-center space-x-2">
                              ...
                              <div>
                                <p className="text-sm">
                                  {invitation.inviter.name}'s invitation to{" "}
                                  {invitation.group.name}
                                </p>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-xl shadow p-4">
              <h3 className="font-medium mb-2 border-b pb-4">
                Group Statistics
              </h3>
