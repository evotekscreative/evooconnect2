  ...
  const checkStatus = async () => {
    try {
      ...
      const response = await fetch(
        `${apiUrl}/api/admin/company-submissions/status/${statusFilter}?limit=${rowsPerPage}&offset=${(currentPage - 1) * rowsPerPage}`,
        {
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 text-center px-4">
      <div className="bg-white p-8 rounded-lg shadow-md max-w-md w-full relative">
        ...
        {isLoading ? (
          ...
          <div className={`border rounded-md p-4 ${status === "approved"
            ? "bg-green-50 border-green-200"
            : status === "rejected"
              ? "bg-red-50 border-red-200"
              : "bg-yellow-50 border-yellow-200"
            }`}>
            <p className={`text-sm ${status === "approved"
              ? "text-green-700"
              : status === "rejected"
                ? "text-red-700"
                : "text-yellow-700"
            ...
            <p className={`text-xs mt-1 ${status === "approved"
              ? "text-green-600"
              : status === "rejected"
                ? "text-red-600"
                : "text-yellow-600"
