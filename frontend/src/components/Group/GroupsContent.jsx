...
export default function GroupsContent({
  ...
  const fetchJoinRequests = async () => {
    try {
      ...
      const response = await axios.get(`${base_url}/api/my-join-requests`, {
        headers: {
  ...
  const handleJoinGroup = async (groupId, isPrivate) => {
    try {
      ...
      showAlert(
        "error",
        error.response?.data?.message || "Failed to request joining the group"
      );
    } finally {
            ...
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                {allGroups.map((group) => {
                  ...
                  const hasJoined =
                    isAdmin || joinedGroups.some((g) => g.id === group.id);
